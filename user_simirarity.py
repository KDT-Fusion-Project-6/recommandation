...
def selection_to_rating(data):
    ...
    for i in range(len(data)):
        codi = data.iloc[i]['codiId']
        user = data.iloc[i]['userId']
...
#             codi_user_dict[codi].append(user)
    ...
    rating_df = pd.DataFrame(selected, columns = ['userId', 'codiId', 'rating'])
    return rating_df, user_codi_dict


def user_link(data, user, k=5):
    a=data.pivot_table('rating', index='userId', columns='codiId')
    ...
    userlink=item_sim_df[user].sort_values(ascending=False)[:k]
